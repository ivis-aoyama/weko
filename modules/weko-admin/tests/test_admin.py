...
class TestsReindexElasticSearchView:
    ...
    def test_ReindexElasticSearchView_index_acl(self, client,users,admin_settings,index, is_permission ,status_code,mocker):
    ...
    def test_ReindexElasticSearchView_reindex_acl(self, client,users,index,admin_settings, is_permission ,status_code,mocker):
        login_user_via_session(client,email=users[index]["email"])
    ...
    def test_ReindexElasticSearchView_reindex_param1(self, client,users,admin_settings,mocker):
        login_user_via_session(client,email=users[0]["email"])# sysadmin
        url = url_for("reindex_es.reindex" , is_db_to_es=False)
        with patch("weko_admin.admin.is_reindex_running", return_value=False):
            mocker.patch("weko_admin.admin.reindex", return_value='completed')
            ...
            assert res.status_code == 200
            assert json.loads(res.data) == {"responce" : _('completed')}
    def test_ReindexElasticSearchView_reindex_param2(self, client,users,admin_settings,mocker):
        login_user_via_session(client,email=users[0]["email"])# sysadmin
        ...
        with patch("weko_admin.admin.is_reindex_running", return_value=False):
            ...
            assert res.status_code == 200
            assert json.loads(res.data) == {"responce" : _('completed')}
    def test_ReindexElasticSearchView_reindex_param3(self, client,users,admin_settings,mocker):
        login_user_via_session(client,email=users[0]["email"])# sysadmin
        ...
        with patch("weko_admin.admin.is_reindex_running", return_value=False):
            ...
            assert res.status_code == 200
            assert json.loads(res.data) == {"responce" : _('completed')}
    def test_ReindexElasticSearchView_reindex_param4(self, client,users,admin_settings,mocker):
        login_user_via_session(client,email=users[0]["email"])# sysadmin
    ...
    def test_ReindexElasticSearchView_reindex_chk_err(self, client,users,admin_settings,mocker):
        login_user_via_session(client,email=users[0]["email"])# sysadmin
        url = url_for("reindex_es.reindex" , is_db_to_es=False)
        with patch("weko_admin.admin.is_reindex_running", return_value=False):
